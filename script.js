...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (registrationForm) {
        console.log('Registration form found, attaching listener');
        registrationForm.addEventListener('submit', e => {
            e.preventDefault();

            // Create a submit button reference if it doesn't exist, looking for type="submit"
            const submitBtn = registrationForm.querySelector('button[type="submit"]');
            // If button not found (might be outside form or named differently), try generic search
            // In registration.html there should be a submit button, usually at the bottom.
            // If I can't find it easily, I'll just skip the button animation or try to find it.
            ...
            const checkboxes = registrationForm.querySelectorAll('input[name="classes[]"]:checked');

            checkboxes.forEach(checkbox => {
                const className = checkbox.value;
                // Try to find associated role
                // The name format in HTML is role_Wed_1830_Beginner_0 for a class value like "Wed 18:30 Beginner 0"
                // We need to sanitize the class value to match the role input name
                // "Wed 18:30 Beginner 0" -> "role_Wed_1830_Beginner_0"
                const sanitizedName = 'role_' + className.replace(/[^a-zA-Z0-9]/g, '_');
                const roleInput = registrationForm.querySelector(`input[name="${sanitizedName}"]:checked`);
                const role = roleInput ? roleInput.value : 'N/A';

                selectedClasses.push(`${className} (${role})`);
            });
            ...
            const data = {
                ...
                email: rawFormData.get('email'),
                phone: rawFormData.get('phone'),
                sex: rawFormData.get('sex'),
                package: rawFormData.get('package'),
                pricing_tier: rawFormData.get('pricing_tier'),
                activation_date: rawFormData.get('activation_date'),
            ...
            const formSubmitData = {
                _subject: `New Registration: ${data.firstname} ${data.lastname}`,
                _template: 'table',
                _captcha: 'false',
                ...
                Phone: data.phone,
                Sex: data.sex,
                Package: `${data.package} (${data.pricing_tier})`,
            ...
            fetch(formSubmitURL, {
                ...
                .catch(error => {
                    console.error('Error!', error.message);
                    alert('Something went wrong with the registration. Please try again or contact us.');
