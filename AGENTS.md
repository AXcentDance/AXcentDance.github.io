...
1. `<style data-critical="HASH">…</style>` — GENERATED above-the-fold CSS. Never hand-edit; regenerate with the pipeline: `python3 scripts/critical_css.py export` → open `/System/flatten/critical.html`, run `await runCritical()` (it POSTs ids to the sink; see System/flatten/README.md) → `python3 scripts/critical_css.py apply`.
